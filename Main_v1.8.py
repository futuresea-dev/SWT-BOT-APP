...
FORM_CLASS, _ = loadUiType(path.join(path.dirname('__file__'), "UI/Main_v1.8.ui"))
from OtherWindow import Ui_OtherWindow
from Show_Hide import Ui_Trades_ShowHide
from Market_Settings import Ui_MarketSet
from QC_Settings import Ui_QC_Set
from Bot_Settings import Ui_Bot
from Trade_Settings import Ui_TradeSet
from ShowHide_TradeHis import Ui_TradeHis_ShowHide
from Show_Hide_Scan import Ui_Scan_ShowHide
from Show_Hide_TradeLog import Ui_TradeLog_ShowHide
from Show_Hide_IndicLog import Ui_IndicLog_ShowHide
...
class Main(QMainWindow, FORM_CLASS):
    ...
    def initControls(self):
        ...
        qss_file = open('Combinear.qss').read()
    ...
    def getIndicatorParametersForOpt(self):
        try:
            ...
            if spMin.MPT_AppliedPrice == "0" and spMax.MPT_AppliedPrice == "3":
                ...
                self.drpMptAppliedPrice.setCurrentText(
                    dnStrategyParameters.convertAppliedPrice(spMin.MPT_AppliedPrice))
            ...
            #     self.drpEmaxEnabled.setCurrentText("all")
            # else:
            #     self.drpEmaxEnabled.setCurrentText(
            #         dnStrategyParameters.convertAppliedPrice(spMin.EMAX_AppliedPrice))
            ...
            self.txtMptShortEmaPeriod.setText(
                str(spMin.MPT_ShortMAPeriod) + "," + str(spMax.MPT_ShortMAPeriod) + "," + str(
                    spStep.MPT_ShortMAPeriod))
            self.removeZeros(self.txtMptShortEmaPeriod)

            self.txtMptLongEmaPeriod.setText(
                str(spMin.MPT_LongMAPeriod) + "," + str(spMax.MPT_LongMAPeriod) + "," + str(
                    spStep.MPT_LongMAPeriod))
    ...
    def initHandlers(self):
        try:
            ...
            # settings buttons
            self.btn_openIndi.clicked.connect(self.onOtherWindow)
            self.btn_openMarket.clicked.connect(self.MarketSettingsWindow)
            self.btn_openTradet.clicked.connect(self.TradeSettingsWindow)
            self.btn_openQC.clicked.connect(self.QCSettingsWindow)
            self.btn_openBot.clicked.connect(self.BotSettingsWindow)

            self.ShowHide_Trades.clicked.connect(self.HideShowPage)
            self.ShowHide_TradeHistory.clicked.connect(self.HideShowTradeHis)
            self.ShowHide_Scanner.clicked.connect(self.HideShowScan)
            self.ShowHide_TradeLog.clicked.connect(self.HideShowTradeLog)
            self.ShowHide_IndLog.clicked.connect(self.HideShowIndicLog)
        except:
            self.printLog("Exception: initHandlers", traceback.format_exc())


    # Show / Hide Indic Logs

    def HideShowIndicLog(self):
        self.window = QtWidgets.QMainWindow()
        self.HideIndicLog = Ui_IndicLog_ShowHide()
        self.HideIndicLog.setupUi(self.window)
        self.window.show()

        self.HideIndicLog.btn_Symbol.clicked.connect(self.IndicLogSymbolHideShow)
        self.HideIndicLog.btn_Time.clicked.connect(self.IndicLogTimeHideShow)
        self.HideIndicLog.btn_CandleTime.clicked.connect(self.IndicLogCandleTimeHideShow)
        self.HideIndicLog.btn_Price.clicked.connect(self.IndicLogPriceHideShow)
        self.HideIndicLog.btn_R_Roc.clicked.connect(self.IndicLogRROCHideShow)
        self.HideIndicLog.btn_MPT.clicked.connect(self.IndicLogMPTHideShow)
        self.HideIndicLog.btn_Trend.clicked.connect(self.IndicLogTrendHideShow)
        self.HideIndicLog.btn_EMAx.clicked.connect(self.IndicLogEMAxHideShow)
        self.HideIndicLog.btn_VStop.clicked.connect(self.IndicLogVStopHideShow)
        self.HideIndicLog.btn_S_ROC.clicked.connect(self.IndicLogSROCHideShow)
        self.HideIndicLog.btn_R_Signal.clicked.connect(self.IndicLogRSignalHideShow)
        self.HideIndicLog.btn_S_Signal.clicked.connect(self.IndicLogSSignalHideShow)
        self.HideIndicLog.btn_RCount.clicked.connect(self.IndicLogRCountHideShow)
        self.HideIndicLog.btn_SCount.clicked.connect(self.IndicLogSCountHideShow)




    def IndicLogSymbolHideShow(self):
        if self.HideIndicLog.btn_Symbol.isChecked():
            self.HideIndicLog.btn_Symbol.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(0, True)
        else:
            self.HideIndicLog.btn_Symbol.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(0, False)

    def IndicLogTimeHideShow(self):
        if self.HideIndicLog.btn_Time.isChecked():
            self.HideIndicLog.btn_Time.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(1, True)
        else:
            self.HideIndicLog.btn_Time.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(1, False)

    def IndicLogCandleTimeHideShow(self):
        if self.HideIndicLog.btn_CandleTime.isChecked():
            self.HideIndicLog.btn_CandleTime.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(2, True)
        else:
            self.HideIndicLog.btn_CandleTime.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(2, False)

    def IndicLogPriceHideShow(self):
        if self.HideIndicLog.btn_Price.isChecked():
            self.HideIndicLog.btn_Price.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(3, True)
        else:
            self.HideIndicLog.btn_Price.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(3, False)

    def IndicLogRROCHideShow(self):
        if self.HideIndicLog.btn_R_Roc.isChecked():
            self.HideIndicLog.btn_R_Roc.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(4, True)
        else:
            self.HideIndicLog.btn_R_Roc.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(4, False)

    def IndicLogMPTHideShow(self):
        if self.HideIndicLog.btn_MPT.isChecked():
            self.HideIndicLog.btn_MPT.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(5, True)
        else:
            self.HideIndicLog.btn_MPT.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(5, False)

    def IndicLogTrendHideShow(self):
        if self.HideIndicLog.btn_Trend.isChecked():
            self.HideIndicLog.btn_Trend.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(6, True)
        else:
            self.HideIndicLog.btn_Trend.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(6, False)

    def IndicLogEMAxHideShow(self):
        if self.HideIndicLog.btn_EMAx.isChecked():
            self.HideIndicLog.btn_EMAx.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(7, True)
        else:
            self.HideIndicLog.btn_EMAx.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(7, False)

    def IndicLogVStopHideShow(self):
        if self.HideIndicLog.btn_VStop.isChecked():
            self.HideIndicLog.btn_VStop.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(8, True)
        else:
            self.HideIndicLog.btn_VStop.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(8, False)

    def IndicLogSROCHideShow(self):
        if self.HideIndicLog.btn_S_ROC.isChecked():
            self.HideIndicLog.btn_S_ROC.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(9, True)
        else:
            self.HideIndicLog.btn_S_ROC.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(9, False)

    def IndicLogRSignalHideShow(self):
        if self.HideIndicLog.btn_R_Signal.isChecked():
            self.HideIndicLog.btn_R_Signal.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(10, True)
        else:
            self.HideIndicLog.btn_R_Signal.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(10, False)

    def IndicLogSSignalHideShow(self):
        if self.HideIndicLog.btn_S_Signal.isChecked():
            self.HideIndicLog.btn_S_Signal.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(11, True)
        else:
            self.HideIndicLog.btn_S_Signal.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(11, False)

    def IndicLogRCountHideShow(self):
        if self.HideIndicLog.btn_RCount.isChecked():
            self.HideIndicLog.btn_RCount.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(12, True)
        else:
            self.HideIndicLog.btn_RCount.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(12, False)

    def IndicLogSCountHideShow(self):
        if self.HideIndicLog.btn_SCount.isChecked():
            self.HideIndicLog.btn_SCount.setStyleSheet("background-color: red;")
            self.tblIndicatorLogs.setColumnHidden(13, True)
        else:
            self.HideIndicLog.btn_SCount.setStyleSheet("background-color: green;")
            self.tblIndicatorLogs.setColumnHidden(13, False)


    #Show / Hide Trade Logs
    def HideShowTradeLog(self):
        self.window = QtWidgets.QMainWindow()
        self.HideTradeLog = Ui_TradeLog_ShowHide()
        self.HideTradeLog.setupUi(self.window)
        self.window.show()
        self.HideTradeLog.btn_colId.clicked.connect(self.TradeLogIdHideShow)
        self.HideTradeLog.btn_BtId.clicked.connect(self.TradeLogBtIdHideShow)
        self.HideTradeLog.btn_Time.clicked.connect(self.TradeLogTimeCndlHideShow)
        self.HideTradeLog.btn_Synbol.clicked.connect(self.TradeLogSynbolHideShow)
        self.HideTradeLog.btn_Type.clicked.connect(self.TradeLogTypeHideShow)
        self.HideTradeLog.btn_Action.clicked.connect(self.TradeLogActionHideShow)
        self.HideTradeLog.btn_Comment.clicked.connect(self.TradeLogCommentHideShow)
        self.HideTradeLog.btn_BaseAmt.clicked.connect(self.TradeLogBaseAmtHideShow)
        self.HideTradeLog.btn_QuoteAmt.clicked.connect(self.TradeLogQuoteAmtHideShow)
        self.HideTradeLog.btn_EntryPrice.clicked.connect(self.TradeLogEntryPriceHideShow)
        self.HideTradeLog.btn_TargPrice.clicked.connect(self.TradeLogTargHideShow)
        self.HideTradeLog.btn_CurPrice.clicked.connect(self.TradeLogCurPriceHideShow)
        self.HideTradeLog.btn_TSL.clicked.connect(self.TradeLogTSLHideShow)
        self.HideTradeLog.btn_PL.clicked.connect(self.TradeLogPLHideShow)
        self.HideTradeLog.btn_PLpercent.clicked.connect(self.TradeLogPLpercentHideShow)
        self.HideTradeLog.btn_Fees.clicked.connect(self.TradeLogFeeHideShow)

    def TradeLogIdHideShow(self):
        if self.HideTradeLog.btn_colId.isChecked():
            self.HideTradeLog.btn_colId.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(0, True)
        else:
            self.HideTradeLog.btn_colId.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(0, False)

    def TradeLogBtIdHideShow(self):
        if self.HideTradeLog.btn_BtId.isChecked():
            self.HideTradeLog.btn_BtId.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(1, True)
        else:
            self.HideTradeLog.btn_BtId.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(1, False)

    def TradeLogTimeCndlHideShow(self):
        if self.HideTradeLog.btn_Time.isChecked():
            self.HideTradeLog.btn_Time.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(2, True)
        else:
            self.HideTradeLog.btn_Time.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(2, False)

    def TradeLogSynbolHideShow(self):
        if self.HideTradeLog.btn_Synbol.isChecked():
            self.HideTradeLog.btn_Synbol.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(3, True)
        else:
            self.HideTradeLog.btn_Synbol.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(3, False)

    def TradeLogTypeHideShow(self):
        if self.HideTradeLog.btn_Type.isChecked():
            self.HideTradeLog.btn_Type.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(4, True)
        else:
            self.HideTradeLog.btn_Type.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(4, False)

    def TradeLogActionHideShow(self):
        if self.HideTradeLog.btn_Action.isChecked():
            self.HideTradeLog.btn_Action.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(5, True)
        else:
            self.HideTradeLog.btn_Action.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(5, False)

    def TradeLogCommentHideShow(self):
        if self.HideTradeLog.btn_Comment.isChecked():
            self.HideTradeLog.btn_Comment.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(6, True)
        else:
            self.HideTradeLog.btn_Comment.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(6, False)

    def TradeLogBaseAmtHideShow(self):
        if self.HideTradeLog.btn_BaseAmt.isChecked():
            self.HideTradeLog.btn_BaseAmt.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(7, True)
        else:
            self.HideTradeLog.btn_BaseAmt.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(7, False)

    def TradeLogQuoteAmtHideShow(self):
        if self.HideTradeLog.btn_QuoteAmt.isChecked():
            self.HideTradeLog.btn_QuoteAmt.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(8, True)
        else:
            self.HideTradeLog.btn_QuoteAmt.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(8, False)


    def TradeLogEntryPriceHideShow(self):
        if self.HideTradeLog.btn_EntryPrice.isChecked():
            self.HideTradeLog.btn_EntryPrice.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(9, True)
        else:
            self.HideTradeLog.btn_EntryPrice.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(9, False)

    def TradeLogTargHideShow(self):
        if self.HideTradeLog.btn_TargPrice.isChecked():
            self.HideTradeLog.btn_TargPrice.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(10, True)
        else:
            self.HideTradeLog.btn_TargPrice.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(10, False)

    def TradeLogCurPriceHideShow(self):
        if self.HideTradeLog.btn_CurPrice.isChecked():
            self.HideTradeLog.btn_CurPrice.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(11, True)
        else:
            self.HideTradeLog.btn_CurPrice.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(11, False)



    def TradeLogTSLHideShow(self):
        if self.HideTradeLog.btn_TSL.isChecked():
            self.HideTradeLog.btn_TSL.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(12, True)
        else:
            self.HideTradeLog.btn_TSL.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(12, False)



    def TradeLogPLHideShow(self):
        if self.HideTradeLog.btn_PL.isChecked():
            self.HideTradeLog.btn_PL.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(13, True)
        else:
            self.HideTradeLog.btn_PL.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(13, False)

    def TradeLogPLpercentHideShow(self):
        if self.HideTradeLog.btn_PLpercent.isChecked():
            self.HideTradeLog.btn_PLpercent.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(14, True)
        else:
            self.HideTradeLog.btn_PLpercent.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(14, False)

    def TradeLogFeeHideShow(self):
        if self.HideTradeLog.btn_Fees.isChecked():
            self.HideTradeLog.btn_Fees.setStyleSheet("background-color: red;")
            self.tblTradeLogs.setColumnHidden(15, True)
        else:
            self.HideTradeLog.btn_Fees.setStyleSheet("background-color: green;")
            self.tblTradeLogs.setColumnHidden(15, False)

    # Show/Hide Scanner
    def HideShowScan(self):
        self.window = QtWidgets.QMainWindow()
        self.HideScan = Ui_Scan_ShowHide()
        self.HideScan.setupUi(self.window)
        self.window.show()
        self.HideScan.btn_colId.clicked.connect(self.ScanIdHideShow)
        self.HideScan.btn_Symbol.clicked.connect(self.ScanSymbolHideShow)
        self.HideScan.btn_Price.clicked.connect(self.ScanPriceHideShow)
        self.HideScan.btn_LastUpdate.clicked.connect(self.ScanLastUpdateHideShow)
        self.HideScan.btn_Params.clicked.connect(self.ScanParamsHideShow)
        self.HideScan.btn_R_Roc.clicked.connect(self.ScanRROCHideShow)
        self.HideScan.btn_MPT.clicked.connect(self.ScanMPTHideShow)
        self.HideScan.btn_Trend.clicked.connect(self.ScanTrendHideShow)
        self.HideScan.btn_EMAx.clicked.connect(self.ScanEMAxHideShow)
        self.HideScan.btn_VStop.clicked.connect(self.ScanVStopHideShow)
        self.HideScan.btn_S_ROC.clicked.connect(self.ScanSROCHideShow)
        self.HideScan.btn_R_Signal.clicked.connect(self.ScanRSignalHideShow)
        self.HideScan.btn_S_Signal.clicked.connect(self.ScanSSignalHideShow)

    def ScanIdHideShow(self):
        if self.HideScan.btn_colId.isChecked():
            self.HideScan.btn_colId.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(0, True)
        else:
            self.HideScan.btn_colId.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(0, False)

    def ScanSymbolHideShow(self):
        if self.HideScan.btn_Symbol.isChecked():
            self.HideScan.btn_Symbol.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(1, True)
        else:
            self.HideScan.btn_Symbol.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(1, False)

    def ScanPriceHideShow(self):
        if self.HideScan.btn_Price.isChecked():
            self.HideScan.btn_Price.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(2, True)
        else:
            self.HideScan.btn_Price.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(2, False)

    def ScanLastUpdateHideShow(self):
        if self.HideScan.btn_LastUpdate.isChecked():
            self.HideScan.btn_LastUpdate.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(3, True)
        else:
            self.HideScan.btn_LastUpdate.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(3, False)

    def ScanParamsHideShow(self):
        if self.HideScan.btn_Params.isChecked():
            self.HideScan.btn_Params.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(4, True)
        else:
            self.HideScan.btn_Params.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(4, False)

    def ScanRROCHideShow(self):
        if self.HideScan.btn_R_Roc.isChecked():
            self.HideScan.btn_R_Roc.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(5, True)
        else:
            self.HideScan.btn_R_Roc.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(5, False)

    def ScanMPTHideShow(self):
        if self.HideScan.btn_MPT.isChecked():
            self.HideScan.btn_MPT.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(6, True)
        else:
            self.HideScan.btn_MPT.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(6, False)

    def ScanTrendHideShow(self):
        if self.HideScan.btn_Trend.isChecked():
            self.HideScan.btn_Trend.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(7, True)
        else:
            self.HideScan.btn_Trend.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(7, False)

    def ScanEMAxHideShow(self):
        if self.HideScan.btn_EMAx.isChecked():
            self.HideScan.btn_EMAx.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(8, True)
        else:
            self.HideScan.btn_EMAx.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(8, False)

    def ScanVStopHideShow(self):
        if self.HideScan.btn_VStop.isChecked():
            self.HideScan.btn_VStop.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(9, True)
        else:
            self.HideScan.btn_VStop.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(9, False)

    def ScanSROCHideShow(self):
        if self.HideScan.btn_S_ROC.isChecked():
            self.HideScan.btn_S_ROC.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(10, True)
        else:
            self.HideScan.btn_S_ROC.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(10, False)

    def ScanRSignalHideShow(self):
        if self.HideScan.btn_R_Signal.isChecked():
            self.HideScan.btn_R_Signal.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(11, True)
        else:
            self.HideScan.btn_R_Signal.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(11, False)

    def ScanSSignalHideShow(self):
        if self.HideScan.btn_S_Signal.isChecked():
            self.HideScan.btn_S_Signal.setStyleSheet("background-color: red;")
            self.tblScannerMarket.setColumnHidden(12, True)
        else:
            self.HideScan.btn_S_Signal.setStyleSheet("background-color: green;")
            self.tblScannerMarket.setColumnHidden(12, False)

    # SHow/ Hide Trade Histories
    def HideShowTradeHis(self):
        self.window = QtWidgets.QMainWindow()
        self.HideTradeHis = Ui_TradeHis_ShowHide()
        self.HideTradeHis.setupUi(self.window)
        self.window.show()
        self.HideTradeHis.btn_colId.clicked.connect(self.HistoryIdHideShow)
        self.HideTradeHis.btn_Symbol.clicked.connect(self.HistorySymbolHideShow)
        self.HideTradeHis.btn_Strat.clicked.connect(self.HistoryStratHideShow)
        self.HideTradeHis.btn_EnrtyTime.clicked.connect(self.HistoryEnrtyTimeHideShow)
        self.HideTradeHis.btn_ExitTime.clicked.connect(self.HistoryExitTimeHideShow)
        self.HideTradeHis.btn_EntryPrice.clicked.connect(self.HistoryEntryPriceHideShow)
        self.HideTradeHis.btn_ExitPrice.clicked.connect(self.HistoryExitPriceHideShow)
        self.HideTradeHis.btn_BaseAmt.clicked.connect(self.HistoryBaseAmtHideShow)
        self.HideTradeHis.btn_QuoteAmt.clicked.connect(self.HistoryQuoteAmtHideShow)
        self.HideTradeHis.btn_PL.clicked.connect(self.HistoryPLHideShow)
        self.HideTradeHis.btn_PLpercent.clicked.connect(self.HistoryPLpercentHideShow)

    def HistoryIdHideShow(self):
        if self.HideTradeHis.btn_colId.isChecked():
            self.HideTradeHis.btn_colId.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(0, True)
        else:
            self.HideTradeHis.btn_colId.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(0, False)

    def HistorySymbolHideShow(self):
        if self.HideTradeHis.btn_Symbol.isChecked():
            self.HideTradeHis.btn_Symbol.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(1, True)
        else:
            self.HideTradeHis.btn_Symbol.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(1, False)

    def HistoryStratHideShow(self):
        if self.HideTradeHis.btn_Strat.isChecked():
            self.HideTradeHis.btn_Strat.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(2, True)
        else:
            self.HideTradeHis.btn_Strat.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(2, False)

    def HistoryEnrtyTimeHideShow(self):
        if self.HideTradeHis.btn_EnrtyTime.isChecked():
            self.HideTradeHis.btn_EnrtyTime.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(3, True)
        else:
            self.HideTradeHis.btn_EnrtyTime.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(3, False)

    def HistoryExitTimeHideShow(self):
        if self.HideTradeHis.btn_ExitTime.isChecked():
            self.HideTradeHis.btn_ExitTime.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(4, True)
        else:
            self.HideTradeHis.btn_ExitTime.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(4, False)

    def HistoryEntryPriceHideShow(self):
        if self.HideTradeHis.btn_EntryPrice.isChecked():
            self.HideTradeHis.btn_EntryPrice.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(5, True)
        else:
            self.HideTradeHis.btn_EntryPrice.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(5, False)

    def HistoryExitPriceHideShow(self):
        if self.HideTradeHis.btn_ExitPrice.isChecked():
            self.HideTradeHis.btn_ExitPrice.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(6, True)
        else:
            self.HideTradeHis.btn_ExitPrice.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(6, False)

    def HistoryBaseAmtHideShow(self):
        if self.HideTradeHis.btn_BaseAmt.isChecked():
            self.HideTradeHis.btn_BaseAmt.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(8, True)
        else:
            self.HideTradeHis.btn_BaseAmt.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(8, False)

    def HistoryQuoteAmtHideShow(self):
        if self.HideTradeHis.btn_QuoteAmt.isChecked():
            self.HideTradeHis.btn_QuoteAmt.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(9, True)
        else:
            self.HideTradeHis.btn_QuoteAmt.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(9, False)

    def HistoryPLHideShow(self):
        if self.HideTradeHis.btn_PL.isChecked():
            self.HideTradeHis.btn_PL.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(10, True)
        else:
            self.HideTradeHis.btn_PL.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(10, False)

    def HistoryPLpercentHideShow(self):
        if self.HideTradeHis.btn_PLpercent.isChecked():
            self.HideTradeHis.btn_PLpercent.setStyleSheet("background-color: red;")
            self.tblClosedTrades.setColumnHidden(11, True)
        else:
            self.HideTradeHis.btn_PLpercent.setStyleSheet("background-color: green;")
            self.tblClosedTrades.setColumnHidden(11, False)

    # Hide/Show Trade Page
    def HideShowPage(self):
        self.window = QtWidgets.QMainWindow()
        self.hide = Ui_Trades_ShowHide()

        self.hide.setupUi(self.window)
        self.window.show()
        # self.ShowHide = Ui_Trades_ShowHide()
        # self.ShowHide.show()
        self.hide.btn_colId.clicked.connect(self.IdHideShow)
        self.hide.btn_EnrtyTime.clicked.connect(self.EnrtyTimeHideShow)
        self.hide.btn_EntryTimeCndl.clicked.connect(self.EntryTimeCndlHideShow)
        self.hide.btn_LastUpdate.clicked.connect(self.LastUpdateHideShow)
        self.hide.btn_Synbol.clicked.connect(self.SynbolHideShow)
        self.hide.btn_Strat.clicked.connect(self.StratHideShow)
        self.hide.btn_EntryPrice.clicked.connect(self.EntryPriceHideShow)
        self.hide.btn_CurPrice.clicked.connect(self.CurPriceHideShow)
        self.hide.btn_TargPrice.clicked.connect(self.TargHideShow)
        self.hide.btn_TSL.clicked.connect(self.TSLHideShow)
        self.hide.btn_BaseAmt.clicked.connect(self.BaseAmtHideShow)
        self.hide.btn_QuoteAmt.clicked.connect(self.QuoteAmtHideShow)
        self.hide.btn_PL.clicked.connect(self.PLHideShow)
        self.hide.btn_PLpercent.clicked.connect(self.PLpercentHideShow)

    def IdHideShow(self):
        if self.hide.btn_colId.isChecked():
            self.hide.btn_colId.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(0, True)
        else:
            self.hide.btn_colId.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(0, False)

    def EnrtyTimeHideShow(self):
        if self.hide.btn_EnrtyTime.isChecked():
            self.hide.btn_EnrtyTime.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(1, True)
        else:
            self.hide.btn_EnrtyTime.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(1, False)

    def EntryTimeCndlHideShow(self):
        if self.hide.btn_EntryTimeCndl.isChecked():
            self.hide.btn_EntryTimeCndl.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(2, True)
        else:
            self.hide.btn_EntryTimeCndl.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(2, False)

    def LastUpdateHideShow(self):
        if self.hide.btn_LastUpdate.isChecked():
            self.hide.btn_LastUpdate.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(3, True)
        else:
            self.hide.btn_LastUpdate.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(3, False)

    def SynbolHideShow(self):
        if self.hide.btn_Synbol.isChecked():
            self.hide.btn_Synbol.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(4, True)
        else:
            self.hide.btn_Synbol.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(4, False)

    def StratHideShow(self):
        if self.hide.btn_Strat.isChecked():
            self.hide.btn_Strat.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(5, True)
        else:
            self.hide.btn_Strat.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(5, False)

    def EntryPriceHideShow(self):
        if self.hide.btn_EntryPrice.isChecked():
            self.hide.btn_EntryPrice.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(6, True)
        else:
            self.hide.btn_EntryPrice.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(6, False)

    def CurPriceHideShow(self):
        if self.hide.btn_CurPrice.isChecked():
            self.hide.btn_CurPrice.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(7, True)
        else:
            self.hide.btn_CurPrice.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(7, False)

    def TargHideShow(self):
        if self.hide.btn_TargPrice.isChecked():
            self.hide.btn_TargPrice.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(8, True)
        else:
            self.hide.btn_TargPrice.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(8, False)

    def TSLHideShow(self):
        if self.hide.btn_TSL.isChecked():
            self.hide.btn_TSL.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(9, True)
        else:
            self.hide.btn_TSL.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(9, False)

    def BaseAmtHideShow(self):
        if self.hide.btn_BaseAmt.isChecked():
            self.hide.btn_BaseAmt.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(10, True)
        else:
            self.hide.btn_BaseAmt.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(10, False)

    def QuoteAmtHideShow(self):
        if self.hide.btn_QuoteAmt.isChecked():
            self.hide.btn_QuoteAmt.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(11, True)
        else:
            self.hide.btn_QuoteAmt.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(11, False)

    def PLHideShow(self):
        if self.hide.btn_PL.isChecked():
            self.hide.btn_PL.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(12, True)
        else:
            self.hide.btn_PL.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(12, False)

    def PLpercentHideShow(self):
        if self.hide.btn_PLpercent.isChecked():
            self.hide.btn_PLpercent.setStyleSheet("background-color: red;")
            self.tblTrades.setColumnHidden(13, True)
        else:
            self.hide.btn_PLpercent.setStyleSheet("background-color: green;")
            self.tblTrades.setColumnHidden(13, False)

    # QC Parameters
    def onOtherWindow(self):
        self.window = QtWidgets.QMainWindow()
        self.ui = Ui_OtherWindow()
        self.ui.setupUi(self.window)
        self.window.show()

    def MarketSettingsWindow(self):
        self.window = QtWidgets.QMainWindow()
        self.marketwindow = Ui_MarketSet()
        self.marketwindow.setupUi(self.window)
        self.window.show()

    def TradeSettingsWindow(self):
        self.window = QtWidgets.QMainWindow()
        self.tradewindow = Ui_TradeSet()
        self.tradewindow.setupUi(self.window)
        self.window.show()

    def QCSettingsWindow(self):
        self.window = QtWidgets.QMainWindow()
        self.qcwindow = Ui_QC_Set()
        self.qcwindow.setupUi(self.window)
        self.window.show()

    def BotSettingsWindow(self):
        self.window = QtWidgets.QMainWindow()
        self.botwindow = Ui_Bot()
        self.botwindow.setupUi(self.window)
        self.window.show()
    ...
    def initClosedTradeDatagrid(self, resultList, datagrid):
        try:
            ...
            for item in resultList:
                ...
                datagrid.setItem(row_number, k, QTableWidgetItem(targetPriceStr))
                # datagrid.setItem(row_number, k, MyTableWidgetItem("%s" %(targetPriceStr), targetPriceStr))
                k = k + 1

                # it.setData(Qt.DisplayRole, self.formatAmount(s, item.Amount))
                datagrid.setItem(row_number, k, QTableWidgetItem(self.formatAmount(s, item.Amount)))

                # datagrid.setItem(row_number, k, it)
                k = k + 1
                # it.setData(Qt.DisplayRole, self.formatPrice(s, item.QuoteAmount))
                datagrid.setItem(row_number, k, QTableWidgetItem(self.formatPrice(s, item.QuoteAmount)))
                # datagrid.setItem(row_number, k, it)
                k = k + 1

                # it.setData(Qt.DisplayRole, self.formatPrice(s, item.PLAmount))
                datagrid.setItem(row_number, k, QTableWidgetItem(self.formatPrice(s, item.PLAmount)))
                # datagrid.setItem(row_number, k, it)
                if item.PLAmount > 0:
                    datagrid.item(row_number, k).setBackground(QBrush(self.trueColor))
...
def main():
    ...
    Main.window = Main()
    # Main.window.setMinimumSize(1327, 905)
    Main.window.show()
